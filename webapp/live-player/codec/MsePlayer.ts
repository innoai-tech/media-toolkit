export class MP4fSourceBuffer {
  static create = (mediaSource: MediaSource, codecs: string): MP4fSourceBuffer => {
    ...
    return new MP4fSourceBuffer(mediaSource, mediaSource.addSourceBuffer(mimeCodec));
  };
  ...
  private appendSourceBuffer = (buf: ArrayBuffer) => {
    try {
  ...
  private load = () => {
    if (!this.sourceBuffer.updating) {
      if (this.buffer.length > 0) {
        this.appendSourceBuffer(this.buffer.shift()!);
      } else {
...
export class MsePlayer {
  ...
  load(url: string) {
    ...
    this.effect(() => {
      const onError = () => {
        console.log("Error " + this.$video.error!.code + "; details: " + this.$video.error!.message);
      };
  ...
  private start = () => {
    this.effect(() => {
      ...
      ws.onmessage = (event) => {
        ...
        if (data[0] === 9) {
          buf = MP4fSourceBuffer.create(this.ms, new TextDecoder("utf-8").decode(data.slice(1)));
        } else {
          buf?.writePacket(event.data);
        }
        // when browser hidden video will be passed.
        // so need to manual set time
