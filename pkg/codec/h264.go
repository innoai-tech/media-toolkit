...
func NewH264Decoder(avcDecoderConfRecord []byte, width, height int) (*H264Decoder, error) {
	if len(avcDecoderConfRecord) == 0 {
		return nil, fmt.Errorf("h264 decoder need AVCDecoderConfRecord")
	}
	...
	return &H264Decoder{codecCtx: codecCtx, frame: frame, width: width, height: height}, nil
}

func (d *H264Decoder) Info() Info {
...
func (d *H264Decoder) DecodeToImage(nalu []byte) (*image.YCbCr, error) {
	avPacket := C.AVPacket{}
