...
import { BlobInfo, deleteBlob, listBlob } from "webapp/client/livestream";
import {
  formatLabelQL, parseLabelQL, stringifySearch,
  Subscribe,
  useObservableEffect,
  useRequest,
  useStateSubject
...
export const BlobInfoQuerier = () => {
  ...
  const filterValue$ = useStateSubject<FilterValue>(() => !isEmpty(valuesFromSearch) ? valuesFromSearch : {
  ...
  useObservableEffect(() => {
    return merge(
      ...
      deleteBlob$.pipe(
        tap((resp) =>
          list$.next((list) =>
            list.filter((b) => b.ref != resp.config.inputs.ref)
          )
        )
      )
    );
