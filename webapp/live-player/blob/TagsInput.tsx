...
import { StateSubject, Subscribe, useObservable, useObservableEffect, useRequest, useStateSubject } from "webapp/utils";
  ...
  useObservableEffect(
    () =>
      merge(
        ...
        unLabelBlob$.pipe(
          tap((resp) => {
            tags$.next((values) => values.filter((v) => v != resp.config.inputs.value));
          })
        )
      ),
    []
