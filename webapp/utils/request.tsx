...
import { mergeMap, Subject, from, Observable, map, merge, catchError, of, tap, filter, Subscriber } from "rxjs";
import { useObservable, useObservableEffect } from "./hooks";
...
export const isContentTypeFormURLEncoded = (headers: any) => getContentType(headers).includes("application/x-www-form-urlencoded");
...
export const applyRequestInterceptors = (...requestInterceptors: RequestInterceptor[]) => (fetcher: Fetcher) => {
  return {
...
export const FetcherProvider = ({ fetcher, children }: { fetcher: Fetcher, children: ReactNode }) => {
  return (
...
class RequestSubject<TInputs, TBody, TError> extends Observable<FetcherResponse<TInputs, TBody>> {
  ...
  constructor(private fetcher: Fetcher, private createConfig: RequestConfigCreator<TInputs, TBody>) {
    ...
    this._input$.pipe(
      mergeMap((input) => from(fetcher.request<TInputs, TBody>(createConfig(input)))),
...
export const useRequest = <TReq, TRespData>(createConfig: RequestConfigCreator<TReq, TRespData>) => {
  const fetcher = useFetcher();
  return useMemo(() => {
    return new RequestSubject<TReq, TRespData, RespError>(fetcher, createConfig);
  }, [fetcher]);
};
