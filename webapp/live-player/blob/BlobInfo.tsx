import { Box, Divider, Grid, IconButton, Paper, Stack, useMediaQuery } from "@mui/material";
import { Sync, Download } from "@mui/icons-material";
import { formatRFC3339, parseISO } from "date-fns";
import { BlobInfo, deleteBlob, exportDataset, listBlob } from "webapp/client/livestream";
import {
  formatLabelQL,
  parseLabelQL,
  stringifySearch,
...
import { DefaultDateRange, FilterBuilder, FilterValue, LabelMeta } from "./FilterBuilder";
...
const parseTimeRange = (timeRange: string): [Date | number, Date | number] | null => {
  try {
    const values = timeRange.split("..").map((t) => parseISO(t));
    return values.length == 2 ? (values as [Date | number, Date | number]) : null;
  } catch (_) {
    return null;
  }
};
...
export const BlobInfoQuerier = () => {
  ...
  useObservableEffect(() => {
    return merge(
          ...
          navigate(
            {
              search: stringifySearch({
                time: [formatTimeRange(nextFilterValue.time[0], nextFilterValue.time[1])],
      ...
      deleteBlob$.pipe(tap((resp) => list$.next((list) => list.filter((b) => b.ref != resp.config.inputs.ref))))
    );
  }, []);
      ...
      <Paper sx={{ maxWidth: 1200 }}>
        ...
        >
          ...
          <Subscribe value$={listBlob$.requesting$}>
            {(requesting) => (
              <IconButton disabled={requesting} onClick={() => fetch(filterValue$.value)}>
                <Sync />
              </IconButton>
            )}
          </Subscribe>
