...
/*
	...
	static int h265dec_new(h265decoder_t *dec, uint8_t *data, int len) {
		dec->codec = avcodec_find_decoder(AV_CODEC_ID_H265);
	...
	static int h265dec_close(h265decoder_t *dec) {
		av_frame_free(&dec->frame);
		avcodec_close(dec->ctx);
		return 0;
	}

	static int h265dec_decode(h265decoder_t *dec, uint8_t *data, int len) {
...
func NewH265Decoder(header []byte) (dec *H265Decoder, err error) {
	dec = &H265Decoder{}
	r := C.h265dec_new(
		&dec.dec,
...
func (m *H265Decoder) Close() error {
	_ = C.h265dec_close(&m.dec)
	return nil
}

func (m *H265Decoder) DecodeToImage(nal []byte) (f *image.YCbCr, err error) {
	defer func() {
		if e := recover(); e != nil {
			if ee, ok := e.(error); ok {
				err = ee
